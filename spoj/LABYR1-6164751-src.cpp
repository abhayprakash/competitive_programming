...
int dfs(int matrix[][1000],int r,int c)
{
    if(matrix[r][c]==0)
      return 0;
    matrix[r][c]=0;
    return 1+maxm(dfs(matrix,r-1,c),dfs(matrix,r+1,c),dfs(matrix,r,c-1),dfs(matrix,r,c+1));
}
int main()
{
    int t;
          ...
          for(int j=1;j<row-1;j++)
            {
                if(matrix[i][j]==1)
                {
                    int lth=dfs(matrix,i,j)-1;
                    if(lth>length)
                      length=lth;
                    //break;
                }
            }
        cout<<"Maximum rope length is "<<length<<"."<<endl;
    }
}*/
#include<iostream>
#include<fstream>
